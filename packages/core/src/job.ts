...
// Job class
export class Job extends (require("events").EventEmitter as new () => TypedEmitter<JobEventMap>) {
  ...
  public async start(): Promise<void> {
    try {
      ...
      switch (this.opts.type) {
        case "convert":
          if (this.opts.format) command = command.output(this.opts.output!).format(this.opts.format);
          else command = command.output(this.opts.output!);
          break;

        case "compress":
          command = command.output(this.opts.output!).videoBitrate(this.opts.bitrate || "1000k");
          break;
        ...
        case "extractAudio":
          command = command.noVideo().output(this.opts.output!).audioCodec(this.opts.codec || "aac");
          break;
        ...
        case "toGif":
          ...
          if (this.opts.duration) command = command.setDuration(this.opts.duration);
          command = command.output(this.opts.output!);
          break;

        case "clip":
          command = command.setStartTime(this.opts.start!).setDuration(this.opts.duration!).output(this.opts.output!);
          break;

        case "concat":
          command = command.input(`concat:${this.opts.input}`).output(this.opts.output!);
          break;
      ...
      (command as any)
        ...
        .on("progress", (progress: ProgressInfo & { timemark?: string }) => {
          if (this.durationSeconds && progress.timemark) {
            progress.percent = Math.min(parseTimeToSeconds(progress.timemark) / this.durationSeconds * 100, 100);
          }
